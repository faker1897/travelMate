...
class SuperBizAgentApp {
    ...
    renderStreamingMessage(messageElement, content) {
    ...
    bindEvents() {
        ...
        if (this.aiOpsSidebarBtn) {
            this.aiOpsSidebarBtn.addEventListener('click', () => this.triggerTravelAdvisor());
        }
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            this.messageInput.placeholder = '问问旅行顾问';
        }
    }
    ...
    // 发送旅行顾问入口请求
    async sendTravelAdvisorRequest(loadingMessageElement, question = '') {
        try {
            const response = await fetch(`${this.apiBaseUrl}/travel_advisor`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    // 更新旅行顾问消息（带折叠详情）
    updateTravelAdvisorMessage(messageElement, response, details) {
        if (!messageElement) {
            // 如果没有传入消息元素，则创建新消息
            return this.addTravelAdvisorMessage(response, details);
        }
        ...
        if (details && details.length > 0) {
            ...
            detailsToggle.innerHTML = `
                ...
                <span>查看规划过程 (${details.length}条)</span>
            `;
    ...
    // 添加旅行顾问消息（带折叠详情）- 保留用于兼容性
    addTravelAdvisorMessage(response, details) {
        const messageDiv = document.createElement('div');
    ...
    async triggerTravelAdvisor() {
        ...
        let question = '';
        if (this.messageInput) {
            question = this.messageInput.value.trim();
        }
        ...
        // 添加"规划中..."的消息（带旋转动画）
        const loadingMessage = this.addLoadingMessage('规划中...');
        ...
        try {
            ...
            console.error('旅行顾问入口请求失败:', error);
            // 更新消息为错误信息
            if (loadingMessage) {
                const messageContent = loadingMessage.querySelector('.message-content');
                if (messageContent) {
                    messageContent.textContent = '抱歉，旅行顾问暂时出错了：' + error.message;
                }
            }
        } finally {
            this.isStreaming = false;
    ...
    showLoadingOverlay(show) {
        if (this.loadingOverlay) {
            if (show) {
                ...
                if (loadingText) loadingText.textContent = '旅行顾问思考中，请稍候...';
                if (loadingSubtext) loadingSubtext.textContent = '正在整理你的旅行需求与建议';
                // 防止页面滚动
